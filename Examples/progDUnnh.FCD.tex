...
\node(vert1)[start]{}; &\\
\node(vert9)[process]{x:=0}; &\\
\node(vert8)[process]{x:=3}; &\\
\node(vert7)[decision]{$z=x$}; &\\
\node(vert6)[process]{z:=0}; &\node(vert5)[process]{z:=x};\\
\node(vert4)[process]{y:=x}; &\\
\node(vert3)[process]{x:=y+z}; &\\
\node(vert2)[stop]{}; &\\
};

\draw [myarrow]  (vert9.south) -- (vert8.north);
\draw [myarrow]  (vert7.south)  -- node[sloped,above]{yes} (vert6.north);
\draw [myarrow]  (vert7.south) -| node[sloped,above, near start]{no} (vert5.north);
\draw [myarrow]  (vert8.south) -- (vert7.north);
\draw [myarrow]  (vert6.south) -- (vert4.north);
