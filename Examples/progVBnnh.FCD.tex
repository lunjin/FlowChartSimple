...
\node(vert1)[start]{}; &\\
\node(vert7)[decision]{$a>b$}; &\\
\node(vert6)[process]{x:=b-a}; &\node(vert4)[process]{y:=b-a};\\
\node(vert5)[process]{y:=a-b}; &\node(vert3)[process]{x:=a-b};\\
\node(vert2)[stop]{}; &\\
};
...
\draw [myarrow]  (vert7.south) -| node[sloped,above, near start]{no} (vert4.north);
\draw [myarrow]  (vert1.south) -- (vert7.north);
\draw [myarrow]  (vert5.south) -- (vert2.north);
