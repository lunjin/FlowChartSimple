...
{ 
    ...
    if(axis == lower.axis) 
      ...
      { result.nodeMatrix = nodeMatrix.joinBottom(lower.nodeMatrix.growLeft(axis-lower.axis));
         result.axis = axis;  
      }
    else /* axis < lower.axis */
      { result.nodeMatrix = nodeMatrix.growLeft(lower.axis-axis).joinBottom(lower.nodeMatrix);
         result.axis = lower.axis;  
      }

    result.links = links; 
    for (Link l:lower.links) result.links.push_back(l);
    result.links.push_back(Link(getExitNode(), lower.getEntryNode(), "->", "--"));
...
{ 
  ...
  unsigned lexit = getExitNode(); // exit node of the then branch
  unsigned rexit = right.getExitNode(); // exit node of the else branch

  unsigned exit  = FCD::registrar.put("[point]{}"); // create decision node
  ...
  result.links.push_back(Link(lexit, exit, " ", " |- "));
  result.links.push_back(Link(rexit, exit, " ", " |- "));

  result.axis = ( nodeMatrix.getWidth() + right.nodeMatrix.getWidth()) /2 ;  
  ...
  result.nodeMatrix.setElement(result.nodeMatrix.getHeight()-1,result.axis, exit); 
...
FCD FCD::whileComp(string cond) const
{
  ...
  result.links.push_back(Link(bexit, lpoint, " ", " -- "));
  result.links.push_back(Link(lpoint, entry, "->", " |- "));
  result.links.push_back(Link(rpoint, exit, " ", " |- "));
  ...
  result.nodeMatrix.setElement(0,result.axis, entry); 
  result.nodeMatrix.setElement(result.nodeMatrix.getHeight()-1,result.axis, exit); 
  result.nodeMatrix.setElement(result.nodeMatrix.getHeight()-2,0,lpoint); 
